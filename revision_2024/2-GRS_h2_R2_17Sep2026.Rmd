...
Last updated Sep 17, 2026 by Samantha Schaffner: Including GRS odds ratio calculation.
...
get_nagelkerke <- function(data, indices) {
  ...
  fit1 <- glm(PD ~ GRS_scaled + sex + age + geno_V3 + geno_V4 + geno_V5, boot_data, family = "binomial")
  fit1_null <- glm(PD ~ sex + age + geno_V3 + geno_V4 + geno_V5, boot_data, family = "binomial")
...
#Calculate OR for quartiles
quartiles <- quantile(meta$GRS_scaled, probs = c(0.25, 0.75), na.rm = TRUE)
# Create a subset: keep only bottom (Q1) and top (Q4) quartiles
meta_sub <- meta[meta$GRS_scaled <= quartiles["25%"] | meta$GRS_scaled >= quartiles["75%"], ]

# Create a binary group variable: 1 = top quartile, 0 = bottom quartile
meta_sub$top_vs_bottom <- as.integer(meta$GRS_scaled[meta$GRS_scaled <= quartiles["25%"] | meta$GRS_scaled >= quartiles["75%"]] >= quartiles["75%"])

model <- glm(PD ~ top_vs_bottom + sex + age + geno_V3 + geno_V4 + geno_V5, data = meta_sub, family = binomial)

# Extract OR and 95% CI
or <- exp(coef(model)["top_vs_bottom"])
ci <- exp(confint(model)["top_vs_bottom", ])

cat(sprintf("OR = %.2f (95%% CI: %.2f–%.2f)\n", or, ci[1], ci[2]))
#OR = 4.31 (95% CI: 1.73–11.61)
```
...
get_nagelkerke_strat <- function(data, indices) {
  boot_data <- data[indices, ]
  fit1 <- glm(PD ~ GRS_scaled + age + geno_V3 + geno_V4 + geno_V5, boot_data, family = "binomial")
  fit1_null <- glm(PD ~ age + geno_V3 + geno_V4 + geno_V5, boot_data, family = "binomial")
  r2_out <- suppressWarnings(nagelkerke(fit1, null=fit1_null))
  nagelkerke_val <- as.numeric(r2_out$Pseudo.R.squared.for.model.vs.null[3])
  return(nagelkerke_val)
}

boot_results <- boot(data = meta[meta$sex == "M", ], statistic = get_nagelkerke_strat, R = 1000, strata = as.factor(meta[meta$sex == "M", "PD"]))
boot.ci(boot_results, type = "bca")
#Level       BCa          
#95%   ( 0.0011,  0.1803 )  
...
boot_results <- boot(data = meta[meta$sex == "F", ], statistic = get_nagelkerke_strat, R = 1000, strata = as.factor(meta[meta$sex == "F", "PD"]))
boot.ci(boot_results, type = "bca")
#Level       BCa          
#95%   ( 0.0001,  0.1449 )  
...
png("~/PD_GRS/prs_plots_by_PD_sex_8Mar2026.png", width=600, height=500)
ggboxplot(
  to_plot,
  x = "PD",
  y = "GRS_scaled",
...
dev.off()
```
![GRS by PD status and sex](/home1/NEURO/schaffner/PD_GRS/prs_plots_by_PD_sex_8Mar2026.png)
