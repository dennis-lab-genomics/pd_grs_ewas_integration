...
Last updated July 3, 2024 by Samantha Schaffner
...
prs_fit_female <- lmFit(mvals[,match(unlist(meta_sub[meta_sub$reportedSex=="F","Sample_Name"]), colnames(mvals))], design_prs_female) #Coefficients not estimable: plate6 plate7 
prs_fit_female <- eBayes(prs_fit_female)

design_pd <- model.matrix(~ 1 + PD + SCORE1_AVG + Age + GPC1 + GPC2 + GPC3 + CTP_PC1 + CTP_PC2 + CTP_PC3 + CTP_PC4 + plate + SentrixPosition_A + reportedSex, data = meta_sub)
...
save(pd_res, prs_res, control_res, case_res, male_res, female_res, file="~/PD_GRS/DIGPD_EWAS_limma_all.RData")
write.csv(pd_res, file="~/PD_GRS/DIGPD_EWAS_PD.csv", row.names=F)
write.csv(prs_res, file="~/PD_GRS/DIGPD_EWAS_GRS_cross_sex.csv", row.names=F)
...
write.csv(prs_hits, file="~/PD_GRS/DIGPD_EWAS_GRS_cross_sex_hits.csv", row.names=F)
...
prs_hits$UCSC_REFGENE_NAME <- c("","KANSL1","","ARHGAP27","KANSL1","KANSL1","","MAPT","KANSL1","MAPT","KIAA1267",
                                "MAPT","")
...
write.csv(prs_res, file="~/PD_GRS/DIGPD_EWAS_GRS_cross_sex.csv", row.names=F)
write.csv(prs_res_hits, file="~/PD_GRS/DIGPD_EWAS_GRS_cross_sex_hits.csv", row.names=F)
...
terre_prs_hits <- read.csv("~/PD_GRS/EWAS_GRS_cross_sex_hits.csv")
length(prs_res_hits$TargetID[prs_res_hits$TargetID %in% terre_prs_hits$TargetID]) #11
colnames(terre_prs_hits)[2:ncol(terre_prs_hits)] <- paste("terre", colnames(terre_prs_hits)[2:ncol(terre_prs_hits)], sep="_")
overlap <- left_join(terre_prs_hits[,c("TargetID","terre_delta_M","terre_adj.P.Val")], prs_res_hits[,c("TargetID","delta_M","adj.P.Val", "UCSC_REFGENE_NAME")], by="TargetID")
write.csv(overlap, file="~/PD_GRS/EWAS_cross_sex_hits_TERRE_DIGPD.csv", row.names=F)
```
![Updated EWAS model - DIGPD](/home1/NEURO/schaffner/PD_GRS/volcano_DIGPD_GRS_cross_sex_Mvals.png)

13 probes are significant in the DIGPD cross-sex sample, 11 of which overlapped with TERRE. In other words, 11 of 27 probes from TERRE (41%) replicated in DIGPD.
