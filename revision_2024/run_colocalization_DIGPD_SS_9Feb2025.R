...
### re-formatting small mqtl data frame to resemble ~/PD_GRS/mQTL/cis_mQTL_out_DIGPD.txt
# columns: SNP, "gene" (CpG), beta, t-stat, p-value, FDR
mqtl <- fread('~/PD_GRS/posthoc/mqtl_digpd_sig_anno.csv')
mqtl <- mqtl[,c("snp","ID","B","t",'P.Value',"adj.P.Val")]
colnames(mqtl) <- c("SNP", "gene","beta", "t-stat", "p-value", "FDR")
write.csv(mqtl, file="~/PD_GRS/coloc/mqtl_digpd_sig_coloc.csv", row.names=F)
...
#setDTthreads(1)
 ...
 smr_mqtl = FALSE,
 mqtl = "~/PD_GRS/coloc/mqtl_digpd_sig_coloc.csv",
...
localize_per_locus <- function(i,
  ...
  probe_res <- mclapply(
    ...
    function(j) {
      ...
      ## SS update 01/10/2025: matching formats of "SNP" columns
      sum_stats$SNP_pos <- paste0("chr",sum_stats$CHR, ":", sum_stats$POS)
      tmp_stats <- na.omit(sum_stats[paste0(coords$CHR, ":", coords$POS), on = "SNP"][!duplicated(SNP)]) #SS 1/11/2025: added removal of duplicate snps
      
      # match up data
      ## SS update 01/10/2025: matching formats of "SNP" columns
      coords$SNP_pos <- paste0(coords$CHR, ":", coords$POS)
      coords <- coords[match(tmp_stats$SNP, coords$SNP_pos),] #SS 12/29/2024: changed "SNP" to "SNP_pos" here
      ...
      if (length(gwas_pval$snp) != 0 | length(mqtl_pval$snp) != 0) { #SS 01/11/2025: changed brackets here
        sink()
...
nb #from coloc.abf documentation:
...
fwrite(
  ...
  glue("{arguments$out}_digpd_sig_pd_snp_colocalization_ph4.txt.gz"),
...
fwrite(
  ...
  glue("{arguments$out}_digpd_sig_pd_snp_colocalization_per_snp.txt.gz"),
  sep = "\t", row.names = F, quote = F
)
