...
#setDTthreads(1)
 ...
 smr_mqtl = FALSE,
 mqtl = "~/PD_GRS/mQTL/cis_mQTL_out.txt.gz",
...
localize_per_locus <- function(i,
                               ...
                               snp_pos = snp_pos,
                               has_methy = TRUE,
                               N1 = 218, #SS: what is N1? Should it be the same as "n" (218)?
                               sum_stats = sum_stats,
...
if (arguments$smr_mqtl) {
  ...
  snp_pos <- fread("~/PD_GRS/mQTL/snp_pos.txt", key = "SNP")
}

setkey(mQTL_results, "gene")
probe_pos <- fread("~/PD_GRS/mQTL/probe_pos.txt")
