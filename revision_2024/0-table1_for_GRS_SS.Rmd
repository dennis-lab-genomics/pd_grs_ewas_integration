...
Last updated Feb 23, 2025 by Samantha Schaffner
...
covars <- covars[, .(SCORE1_AVG, age, alcohol = ifelse(alcohol1 > 2, 1, 0), smoking = ifelse(smoking > 1, 1, 0), pesticides = ifelse(pesticides_work == 1, "Occupational", ifelse(pesticides_domestic == 1, "Domestic", "None")), PD=status, GPC1 = V3, GPC2 = V4, GPC3 = V5, PC1, PC2, PC3, PC4, PC5, PC6, PC7, PC8, PC9, PC10, head_trauma = head_trauma_loc, sex,IID)]
...
or_w_grs <- make_quantiles("Pt_5e-08",prs_mat,covars[,.(PD,sex,age,alcohol,smoking,pesticides=addNA(factor(pesticides,levels=c("None","Domestic","Occupational"))),head_trauma)],4)
...
cross_quant <- get_quantiles("Pt_5e-08",prs_mat,covars2[,.(PD,age,sex,GPC1,GPC2,GPC3,CTP_PC1,CTP_PC2,CTP_PC3,CTP_PC4,CTP_PC5,CTP_PC6,plate,SentrixPosition_A)]) %>% mutate(Sex = "Cross-sex", Threshold="best")
...
## DIGPD table1
```{r}
# SS 23Feb2025: skip this (no access to WC's "genotype_qc" folder)
...
pre_table_data <- digpd_meta2[,.(IID,age, alcohol = ifelse(alcohol > 2, 1, 0), smoking = ifelse(smoking > 1, 1, 0), pesticides = ifelse(pesticides == 2, "Occupational", ifelse(pesticides== 1, "Domestic", "None")), PD, head_trauma, sex)]
...
SS: TO UPDATE THE BELOW SECTION (NEW TABLE W/30 PESTICIDES)
