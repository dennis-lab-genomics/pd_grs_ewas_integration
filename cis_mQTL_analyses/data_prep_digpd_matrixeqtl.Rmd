...
### Sex-stratified data
```{r}
write_delim(t(cbind(digpd_meta[,c("U1","U2","U3","Age","men","CTP_PC1", "CTP_PC2", "CTP_PC3", "CTP_PC4", "CTP_PC5")])) %>% as.data.frame() %>% rownames_to_column("id"),"digpd_data/covariates_CTP.txt")
write_delim(t(cbind(digpd_meta[,c("U1","U2","U3","PD","Age","men","CTP_PC1", "CTP_PC2", "CTP_PC3", "CTP_PC4", "CTP_PC5")])) %>% as.data.frame() %>% rownames_to_column("id"),"digpd_data/covariates_CTP_PD.txt")

write_delim(t(cbind(digpd_meta[digpd_meta$men == 1,c("U1","U2","U3","PD","Age","CTP_PC1", "CTP_PC2", "CTP_PC3", "CTP_PC4", "CTP_PC5")])) %>% as.data.frame() %>% rownames_to_column("id"),"digpd_data/male_covariates_PD_CTP.txt")
write_delim(t(cbind(digpd_meta[digpd_meta$men == 0,c("U1","U2","U3","PD","Age","CTP_PC1", "CTP_PC2", "CTP_PC3", "CTP_PC4", "CTP_PC5")])) %>% as.data.frame() %>% rownames_to_column("id"),"digpd_data/female_covariates_PD_CTP.txt")
...
fwrite(genotype[,..geno_vars],"digpd_data/all_imputed_matrixeQTL.txt",sep = " ",quote = F)
fwrite(genotype[,..geno_vars_f],"digpd_data/female_all_imputed_matrixeQTL.txt",sep = " ",quote = F)
fwrite(genotype[,..geno_vars_m],"digpd_data/male_all_imputed_matrixeQTL.txt",sep = " ",quote = F)

#Methy POS
fwrite(methy_annot[Name %in% rownames(betas_sub),.(geneid=Name, chr=paste0("chr",CHR),s1=MAPINFO,s2=MAPINFO)], "digpd_data/probe_pos.txt",sep = " ",quote=F)
#methy
fwrite(betas_sub %>% as.data.frame() %>%rownames_to_column("cpg"),"digpd_data/methylation_combat.txt",sep=" ",quote=F)
fwrite(betas_sub[,digpd_meta$men == 1] %>% as.data.frame() %>%rownames_to_column("cpg"),"digpd_data/male_methylation_combat.txt",sep=" ",quote=F)
fwrite(betas_sub[,digpd_meta$men == 0] %>% as.data.frame() %>%rownames_to_column("cpg"),"digpd_data/female_methylation_combat.txt",sep=" ",quote=F)
```
