...
Loading data:
```{r}
male_sum_stats <-  fread("~/Blauwendraat_male_female_GWAS/MALE_PD_filtered_sumstats_no_multi_allelics_RSID.QC.txt.gz")
male_sum_stats[,`:=`(SNP=paste0("chr",CHR,":",BP),freq=MAF)]
male_mQTL_results <- fread("terre_data/male_cis_all_impute_mQTL_results_9_methy_PC.txt.gz", key="gene")
# male_loci <- fread("~/prs_ewas_integration/cis_mQTL_analyses/Blauwendraat_male.clumped")
male_loci[,`:=`(POS=BP, `Locus Number`=.I)]

res_male_SNP <- rbindlist(mclapply(1:nrow(male_loci), function(i) localize_per_locus(i, mQTL_results = male_mQTL_results, snp_pos = snp_pos, sum_stats=male_sum_stats, has_methy = FALSE, N1 = 131,risk_scores = male_loci, return_summary = FALSE), mc.cores = 4))
res_male_cpg <- rbindlist(mclapply(1:nrow(risk_scores), function(i) localize_per_locus(i, mQTL_results = male_mQTL_results, snp_pos = snp_pos, sum_stats=male_sum_stats, has_methy = FALSE, N1 = 131,risk_scores = male_loci, return_summary = TRUE), mc.cores = 4))
```
### Female
```{r}
female_sum_stats <-  fread("~/Blauwendraat_male_female_GWAS/FEMALE_PD_filtered_sumstats_no_multi_allelics_RSID.QC.txt.gz")
female_sum_stats[,`:=`(SNP=paste0("chr",CHR,":",BP),freq=MAF)]
female_mQTL_results <- fread("terre_data/female_cis_all_impute_mQTL_results_9_methy_PC.txt.gz", key="gene")
female_loci <- fread("~/prs_ewas_integration/cis_mQTL_analyses/Blauwendraat_female.clumped")
female_loci[,`:=`(POS=BP, `Locus Number`=.I)]

res_female_SNP <- rbindlist(mclapply(1:nrow(female_loci), function(i) localize_per_locus(i, mQTL_results = female_mQTL_results, snp_pos = snp_pos, sum_stats=female_sum_stats, has_methy = FALSE, N1 = 131,risk_scores = female_loci, return_summary = FALSE), mc.cores = 4))
res_female_cpg <- rbindlist(mclapply(1:nrow(risk_scores), function(i) localize_per_locus(i, mQTL_results = female_mQTL_results, snp_pos = snp_pos, sum_stats=female_sum_stats, has_methy = FALSE, N1 = 131,risk_scores = female_loci, return_summary = TRUE), mc.cores = 4))
```
...
```
### Plot @TODO change to plot from coloc output instead for ease of re-plotting
