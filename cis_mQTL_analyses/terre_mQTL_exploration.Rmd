...
### GoDMC mQTL
```{r}
cis_godmc_smr <- fread("~/cis_mqtl_godmc.smr")
all_godmc_smr <- fread("~/all_mqtl_godmc.smr")
```

```{r}
cis_godmc_smr[p_SMR < (0.05 / .N)]
cis_godmc_smr[p_SMR < (0.05 / .N) & p_HEIDI > 0.01]

all_godmc_smr[p_SMR < (0.05 / .N)]
all_godmc_smr[p_SMR < (0.05 / .N) & p_HEIDI > 0.01]
...
all_godmc_smr[p_SMR < (0.05 / .N) & p_HEIDI > 0.01 & probeID %in% prs_hits] # none
all_godmc_smr[p_SMR < (0.05 / .N) & probeID %in% prs_hits] # none
go_dmc_dist_smr <- all_godmc_smr[p_SMR < (0.05 / .N) & p_HEIDI > 0.01][prs_hits_w_pos, on = c("ProbeChr" = "chr"), allow.cartesian = TRUE]
...
all_godmc_smr[p_SMR < (0.05 / .N) & p_HEIDI > 0.01][manifest[, c("name", "UCSC_RefGene_Name", "UCSC_RefGene_Group")], on = c("probeID" = "name"), nomatch = 0][order(p_SMR)]
