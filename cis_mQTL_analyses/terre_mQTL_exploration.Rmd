...
mQTL_results_digpd <- fread("digpd_data/cis_all_impute_mQTL_results_9_methy_PC_no_mut.txt.gz")
```
```{r}
...
```{r}
to_check_replication <- mQTL_results_digpd[mQTL_results[FDR < 0.05], on = c("SNP", "gene"), nomatch = 0]
```

$\pi_1$ statistics
```{r}
(sig_shared <- uniqueN(to_check_replication[FDR < (0.05 / nrow(mQTL_results_digpd))]$gene))

pi0res <- pi0est(na.omit(to_check_replication$`p-value`), lambda = seq(0, 0.245, 0.005), pi0.method = "bootstrap")
1 - pi0res$pi0
plot(pi0res$pi0.lambda, pi0res$lambda, xlab = bquote(pi[0]), ylab = bquote(lambda))
abline(v = pi0res$pi0)
qvalres <- qvalue_truncp(na.omit(to_check_replication$`p-value`))
1 - qvalres$pi0
abline(v = qvalres$pi0)
pi0est(to_check_replication$`p-value` / 0.25)
```
