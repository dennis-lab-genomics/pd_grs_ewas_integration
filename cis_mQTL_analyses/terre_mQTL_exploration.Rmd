...
top_prs_hits %>%
  ...
  left_join(manifest %>%
    ...
    select(name,Gene, `Gene region`), by = c("ID" = "name")) 
```
...
ggplot(all_godmc_smr[p_SMR < (0.05/ .N)],aes(
  ...
  ggrepel::geom_text_repel(
    ...
    aes(label=paste0(probeID,"(",gsub(";.*","",UCSC_RefGene_Name),")")),
    show.legend=F,
    max.overlaps = Inf,
    force = 2
  )+
  ...
  theme_minimal()
ggplot(all_godmc_smr[p_SMR < (0.05/ .N) & probeID %in% godmc_coloc[PP.H4.abf > 0.9]$probe][manifest[, c("name", "UCSC_RefGene_Name", "UCSC_RefGene_Group")], on = c("probeID" = "name"), nomatch = 0]%>% mutate(name=paste0(probeID,"(",gsub(";.*","",UCSC_RefGene_Name),")")) %>% mutate(name=factor(name,levels=name[order(-b_SMR)])),aes(y=name, x=b_SMR,xmin=b_SMR-se_SMR,xmax =b_SMR + se_SMR))+
  geom_point()+
  labs(y="",x="SMR Estimate")+
  geom_errorbarh()+
  theme_minimal()
