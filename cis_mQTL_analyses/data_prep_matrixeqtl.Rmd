...
a1 <- ggplot(results_df,aes(PC,nhit)) + geom_point() +geom_smooth() + geom_hline(yintercept = nrow(ctp_res[FDR < 0.05,.(unique(gene))]),linetype="dashed")  + annotate(geom="text",y=4200,x=10,label="Hits detected with cell type 5 proportion PCs") + theme_classic() + labs(y="# mQTLs Detected", x = "# PCs Included")#+ ggtitle("CpG sites on chr21 with one mQTL Vs Methylation PCs included")
```
