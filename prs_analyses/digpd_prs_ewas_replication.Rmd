...
```{r}
library(table1)
table_data <- meta[prsice_cov$IID,on=.(IID),nomatch=0][visite == 1 | is.na(visite)]
head(table_data)
library(boot)
...
covars_table <- table_data[,.(sex=men,age=Age,head_trauma,smoking,pesticides,status=PD)] %>% mutate(
  ...
  pesticides= factor(ifelse(pesticides > 1,"Exposed","Not exposed"),levels=c("Exposed","Not exposed")),
...
prsice_all <- fread("prsice_data/DIGPD_PRSice.all_score")[match(colnames(methy), IID,nomatch=0), .(FID, IID, `Pt_5e-08`, `Pt_5.005e-05`, `Pt_0.00010005`, `Pt_0.00100005`, `Pt_0.00030005`,`Pt_0.0101501`, `Pt_0.1`, `Pt_0.2`, `Pt_0.3`, `Pt_0.4`, `Pt_0.5`, `Pt_1`)]
covariate <- covariate[match(colnames(methy), covariate$IID,nomatch=0),]
