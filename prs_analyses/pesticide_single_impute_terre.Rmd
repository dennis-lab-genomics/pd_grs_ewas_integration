...
```{r}
ggplot(all_data %>% mutate(PD= ifelse(PD,"CASE","CONTROL")),aes(as.factor(PD),SCORE1_AVG)) +geom_boxplot()+ geom_jitter(height=0, width=0.1)+ labs(y="PRS", x = "PD Status")
...
ggplot(pd_res,aes(logFC,-log10(P.Value)))+ geom_point()
...
ggplot(prs_annot,aes(logFC,-log10(P.Value)))+ geom_point() + geom_text_repel(data=subset(prs_annot,adj.P.Val < 0.05),mapping=aes(logFC,-log10(P.Value),label=gene))
```
...
ggplot(res_pest,aes(logFC,-log10(P.Value)))+ geom_point()
ggplot(res_pest_work,aes(logFC,-log10(P.Value)))+ geom_point()
ggplot(res_pico,aes(logFC,-log10(P.Value)))+ geom_point()
res_pest %>% rownames_to_column("probe") %>% filter(P.Value < (0.05/ n())) %>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name")) %>% filter(P.Value < (0.05/n()))
res_pest_work %>% rownames_to_column("probe") %>% filter(P.Value < (0.05/ n())) %>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name"))%>%filter(P.Value < (0.05/n()))
res_pico %>% rownames_to_column("probe") %>% filter(P.Value < (0.05/ n())) %>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name"))%>%filter(P.Value < (0.05/n()))
```
Checking all exposures:
```{r}
library(caret)
reg_w_all_pesticide <- reg_all_data %>% left_join(pesticide %>% unique(), by=c("patient"="num")) %>% unique() %>% filter(`_imputation_` == 1) %>% select(patient,SCORE1_AVG,age,plate,sex,V3,V4,V5,ins:gardening) %>% mutate_at(vars(ins:gardening),as.factor) %>% select_if(~length(unique(na.omit(.))) > 1) %>% filter_all(all_vars(!is.na(.)))
...
top_10_w_sig_pest%>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name")) %>% mutate(gene = gsub(";.*","",UCSC_RefGene_Name)) %>% filter(adj.P.Val <0.05) %>% select(pesticide,probe,gene,CHR,MAPINFO,P.Value, adj.P.Val) %>% write_delim()
```
...
#### print out table
```{r}
master_pest_interaction_table <- rbindlist(lapply(sig_pesticides_full, function(x)x %>% rownames_to_column(var="probe")%>% filter(adj.P.Val < 0.05)  %>%left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name")) %>% mutate(gene = gsub(";.*","",UCSC_RefGene_Name)) %>% mutate(gene= ifelse(gene == "",probe,gene))), idcol="pesticide")
for(i in 1:nrow(master_pest_interaction_table)){
  ...
  p <- ggplot(tmp_df,aes_string("SCORE1_AVG","avg_beta",color=pest)) + geom_point() + geom_smooth(method="lm") + labs(x = "PRS")
  print(p)
}
```
...
library(ggrepel)
for(pest_col in sig_pesticides){
  annot_data <- sig_pesticides_full[[pest_col]] %>% rownames_to_column(var="probe")%>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name")) %>% mutate(gene = gsub(";.*","",UCSC_RefGene_Name)) %>% mutate(gene= ifelse(gene == "",probe,gene))
  p <- ggplot(annot_data, aes(logFC, -log10(P.Value))) + geom_point() + ggtitle(pest_col) + geom_text_repel(data=subset(annot_data,adj.P.Val < 0.05),mapping=aes(logFC,-log10(P.Value),label=gene))
  print(p)
}
```
...
for(hit in multi_hit){
  annot_data <- sig_pesticides_full[[hit]] %>% rownames_to_column(var="probe")%>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name")) %>% mutate(gene = gsub(";.*","",UCSC_RefGene_Name)) %>% mutate(gene= ifelse(gene == "",probe,gene))
