...
p1 <- ggplot(to_plot %>% mutate(SCORE1_AVG = scale(SCORE1_AVG)), aes(pos,value, color= SCORE1_AVG))+ geom_point() + scale_color_distiller(palette = "Spectral") + theme_minimal() + labs(color="Normalized PD PRS")
p2 <- ggplot(to_plot %>% mutate(PD = ifelse(PD == 1,"CASE","CONTROL")), aes(pos,value, color= PD))+ geom_point() + scale_color_brewer(palette = "Dark2") + theme_minimal()+ stat_summary(aes(group=PD), fun.y=mean, geom="line") + labs(color="PD status")


to_plot <- get_dmr_effects(prs_dmr,2,"chr17",prs_res)

p3 <- ggplot(to_plot %>% mutate(SCORE1_AVG = scale(SCORE1_AVG)), aes(pos,value, color= SCORE1_AVG))+ geom_point() + scale_color_distiller(palette = "Spectral") + theme_minimal()+ labs(color="Normalized PD PRS")
p4 <- ggplot(to_plot %>% mutate(PD = ifelse(PD == 1,"CASE","CONTROL")), aes(pos,value, color= PD))+ geom_point() + scale_color_brewer(palette = "Dark2") + theme_minimal()+ stat_summary(aes(group=PD), fun.y=mean, geom="line") + labs(color="PD status")

to_plot <- get_dmr_effects(prs_dmr,3,"chr17",prs_res)

p5 <- ggplot(to_plot %>% mutate(SCORE1_AVG = scale(SCORE1_AVG)), aes(pos,value, color= SCORE1_AVG))+ geom_point() + scale_color_distiller(palette = "Spectral") + theme_minimal()+ labs(color="PD PRS")
p6 <-  ggplot(to_plot %>% mutate(PD = ifelse(PD == 1,"CASE","CONTROL")), aes(pos,value, color= PD))+ geom_point() + scale_color_brewer(palette = "Dark2") + theme_minimal()+ stat_summary(aes(group=PD), fun.y=mean, geom="line")  + labs(color="Normalized PD PRS")
...
```{r}
prs_res %>% rownames_to_column("probe") %>% filter(adj.P.Val < 0.25) %>% left_join(annot %>% select(Name,CHR,UCSC_RefGene_Name,Relation_to_UCSC_CpG_Island,MAPINFO), by=c("probe"="Name"))%>% mutate(gene = gsub(";.*","",UCSC_RefGene_Name)) %>% mutate(gene= ifelse(gene == "",probe,gene)) %>% write.csv("prs_res_fdr025.csv",row.names = F,quote=F)
