...
```{r}
to_plot <- all_data %>%
  mutate(PD = ifelse(PD, "CASE", "CONTROL"), Sex = ifelse(men == 1, "Male", "Female")) %>%
  mutate(Subjects = factor(paste(PD, Sex), levels = c("CONTROL Male", "CASE Male", "CONTROL Female", "CASE Female")))
ggboxplot(
  to_plot,
  x = "Subjects",
  y = "SCORE1_AVG",
  ...
  stat_compare_means(
    comparisons = list(
      # c("CASE Male","CASE Female"),
      # c("CONTROL Male", "CONTROL Female"),
      c("CASE Male", "CONTROL Male"),
      c("CASE Female", "CONTROL Female")
    )
  ) +
  labs(y = "Polygenic Risk Score", x = "Parkinson's Diagnosis") +
  scale_color_manual(values = c("Male" = "light blue", "Female" = "pink")) +
  ggtitle(bquote("Nagelkerke Pseudo-" * R^2 * " Males 0.0611"))
male_r2$Pseudo.R.squared.for.model.vs.null[3]
ggboxplot(
  to_plot,
  x = "Sex",
  y = "SCORE1_AVG",
...
```
```{r}
prs_dmr <- extractRanges(prs_dmr_res, genome = "hg19")
...
library(cowplot)
get_dmr_effects <- function(dmr, ix, chrom, limma_res) {
  dmr_coord <- as.data.frame(ranges(dmr))[ix, ]
  cpgs <- annotation[annotation$chr == chrom & annotation$pos >= dmr_coord$start & annotation$pos <= dmr_coord$end, ]
...
get_dmr_res <- function(dmr, ix, chrom, limma_res) {
  dmr_coord <- as.data.frame(ranges(dmr))[ix, ]
  cpgs <- annotation[annotation$chr == chrom & annotation$pos >= dmr_coord$start & annotation$pos <= dmr_coord$end, ]
  res <- cbind(cpgs, limma_res[cpgs$Name, ])
  return(as.data.frame(res))
}
to_plot <- get_dmr_effects(prs_dmr, 1, "chr17", prs_res)
...
p2 <- ggplot(to_plot %>% mutate(PD = ifelse(PD == 1, "CASE", "CONTROL")), aes(pos, value, color = PD)) +
  geom_point() +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() +
  stat_summary(aes(group = PD), fun.y = mean, geom = "line") +
  labs(color = "PD status")


to_plot <- get_dmr_effects(prs_dmr, 2, "chr17", prs_res)

p3 <- ggplot(to_plot %>% mutate(SCORE1_AVG = scale(SCORE1_AVG)), aes(pos, value, color = SCORE1_AVG)) +
  geom_point() +
  scale_color_distiller(palette = "Spectral") +
  theme_minimal() +
  labs(color = "Normalized PD PRS")
p4 <- ggplot(to_plot %>% mutate(PD = ifelse(PD == 1, "CASE", "CONTROL")), aes(pos, value, color = PD)) +
  geom_point() +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() +
  stat_summary(aes(group = PD), fun.y = mean, geom = "line") +
...
to_plot <- get_dmr_effects(prs_dmr, 3, "chr17", prs_res)

p5 <- ggplot(to_plot %>% mutate(SCORE1_AVG = scale(SCORE1_AVG)), aes(pos, value, color = SCORE1_AVG)) +
  geom_point() +
  scale_color_distiller(palette = "Spectral") +
  theme_minimal() +
  labs(color = "PD PRS")
p6 <- ggplot(to_plot %>% mutate(PD = ifelse(PD == 1, "CASE", "CONTROL")), aes(pos, value, color = PD)) +
  geom_point() +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() +
  stat_summary(aes(group = PD), fun.y = mean, geom = "line") +
...
plot_grid(p1, p2, p3, p4, p5, p6, byrow = F)
```
...
prs_res %>%
  ...
  mutate(gene = gsub(";.*", "", UCSC_RefGene_Name)) %>%
  mutate(gene = ifelse(gene == "", probe, gene)) %>%
  write.csv("prs_res_fdr025.csv", row.names = F, quote = F)
...
rbindlist(lapply(1:3, function(i) get_dmr_res(prs_dmr, i, "chr17", prs_res))) %>% write.csv("PRS_DMR_CPGs.csv", row.names = F, quote = F)
write.csv(data.frame(prs_dmr@elementMetadata), "PRS_DMR_CPGs_meta.csv", row.names = F, quote = F)
```
...
ggplot(age_res, aes(logFC, -log10(P.Value))) +
  ...
  geom_vline(linetype = "dashed", xintercept = 0.03) +
  geom_vline(linetype = "dashed", xintercept = -0.03)
ggplot(age_res, aes(logFC, -log10(age_main_res[rownames(age_res), ]$P.Value))) +
  geom_point()
```
### PRSxSex interaction
...
ggplot(sex_res, aes(logFC, -log10(P.Value))) +
  geom_point()
ggplot(sex_res, aes(logFC, -log10(sex_main_res[rownames(sex_res), ]$P.Value))) +
  geom_point()
```
