...
betas_sub <- lumi::beta2m(betas_combat)
...
```
### write out covariates for PRSice2 PRS generation
```{r}
IDs <- fread("~/genotype_qc/TERRE_QC/all_imputed_r2_30_rsid_hard_call.fam")[, .(FID = V1, IID = V2)]
tmp_meta <- reg_all_data[match(IDs$IID, IID)]
tmp_meta$PD <- as.numeric(tmp_meta$PD == "CASE")
tmp_meta$sex <- as.numeric(tmp_meta$sex == "Male")
head(prsice_cov <- cbind(model.matrix(~ sex + age + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PD, data = tmp_meta)[, -c(1)], data.frame(IID = tmp_meta$IID, patient = tmp_meta$patient)))
fwrite(prsice_cov, "prsice_cov_and_status_mvalues.txt")
```

#### Differential methylation with PRS vs PD
```{r}
na_row <- !apply(reg_all_data[, c("SCORE1_AVG", "age", "sex")], 1, function(x) any(is.na(x)))
design_prs <- model.matrix(~ 1 + SCORE1_AVG + age + sex + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[na_row, ])
prs_fit <- lmFit(betas_sub[, na_row], design_prs)
prs_fit <- eBayes(prs_fit)
# Cases only
case_select <- na_row & reg_all_data$PD == "CASE"
design_prs_case <- model.matrix(~ 1 + SCORE1_AVG + age + sex + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[case_select, ])
...
design_prs_control <- model.matrix(~ 1 + SCORE1_AVG + age + sex + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[control_select, ])
...
design_prs_male <- model.matrix(~ 1 + SCORE1_AVG + age + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[male_select, ])
...
design_prs_female <- model.matrix(~ 1 + SCORE1_AVG + age + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[female_select, ])
prs_fit_female <- lmFit(betas_sub[, female_select], design_prs_female)
prs_fit_female <- eBayes(prs_fit_female)

design_pd <- model.matrix(~ 1 + PD + age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[na_row, ])
...
```{r}
prs_annotated <- cpg.annotate(datatype = "array", object = betas_sub[, na_row], analysis.type = "differential", design = design_prs, coef = 2, what = "M", arraytype = "EPIC", fdr = 0.05)
prs_dmr_res <- dmrcate(prs_annotated, lambda = 1000, C = 2)

male_prs_annotated <- cpg.annotate(datatype = "array", object = betas_sub[male_select, na_row], analysis.type = "differential", design = design_prs, coef = 2, what = "M", arraytype = "EPIC", fdr = 0.05)
male_prs_dmr_res <- dmrcate(male_prs_annotated, lambda = 1000, C = 2)

female_prs_annotated <- cpg.annotate(datatype = "array", object = betas_sub[female_select, na_row], analysis.type = "differential", design = design_prs, coef = 2, what = "M", arraytype = "EPIC", fdr = 0.05)
female_prs_dmr_res <- dmrcate(female_prs_annotated, lambda = 1000, C = 2)

pd_annotated <- cpg.annotate(datatype = "array", object = betas_sub[, na_row], analysis.type = "differential", design = design_pd, coef = 2, what = "M", arraytype = "EPIC", fdr = 0.05)
...
get_dmr_effects <- function(dmr, limma_res) {
  ...
  dmr_methy <- melt(lumi::m2beta(betas_sub[dmr_1$Name, na_row]), stringsAsFactors = FALSE)
...
write_results <- function(prs_res, prs_dmr, label) {
  prs_res %>%
    ...
    mutate(gene = gsub(";.*", "", UCSC_RefGene_Name)) %>%
    mutate(gene = ifelse(gene == "", probe, gene)) %>%
    write.csv(glue("{label}prs_res_fdr025_mval.csv"), row.names = F, quote = F)

  rbindlist(lapply(prs_dmr$coord, function(dmr) as.data.frame(get_dmr_res(dmr, prs_res)))) %>%
    left_join(prs_dmr, by = c("DMR" = "coord")) %>%
    write.csv(glue("{label}PRS_DMR_CPGs_mval.csv"), row.names = F, quote = F)
}
...
### PRSxAge interaction
```{r}
design_age <- model.matrix(~ 1 + SCORE1_AVG * age + sex + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[na_row, ])
age_fit <- lmFit(betas_sub[, na_row], design_age)
age_fit <- eBayes(age_fit)
age_res <- topTable(age_fit, number = Inf, coef = 18)
age_main_res <- topTable(age_fit, number = Inf, coef = c(2, 18))
```
```{r}
...
```{r}
design_sex <- model.matrix(~ 1 + SCORE1_AVG * sex + age + V3 + V4 + V5 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = reg_all_data[na_row, ])
sex_fit <- lmFit(betas_sub[, na_row], design_sex)
sex_fit <- eBayes(sex_fit)
sex_res <- topTable(sex_fit, number = Inf, coef = 18)
sex_main_res <- topTable(sex_fit, number = Inf, coef = c(2, 18))
```
```{r}
...
(sex_hit <- sex_main_res %>%
  rownames_to_column("probe") %>%
  filter(adj.P.Val < 0.05, !probe %in% prs_hits$probe))
(age_hit <- age_main_res %>%
  rownames_to_column("probe") %>%
  filter(adj.P.Val < 0.05, !probe %in% prs_hits$probe))
ggplot(reg_all_data[na_row, ] %>% mutate(sex = factor(sex, levels = c("Male", "Female"))), aes(SCORE1_AVG, lumi::m2beta(betas_sub[sex_hit$probe[1], ]), color = sex)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  scale_color_manual(values = c("lightblue", "pink")) +
  labs(y = glue("{sex_hit$probe[1]} Methylation"), x = "PD PRS", color = "Sex") +
  theme_minimal()
ggplot(reg_all_data[na_row, ] %>% mutate(sex = factor(sex, levels = c("Male", "Female"))), aes(SCORE1_AVG, lumi::m2beta(betas_sub[sex_hit$probe[2], ]), color = sex)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  scale_color_manual(values = c("lightblue", "pink")) +
  labs(y = glue("{sex_hit$probe[2]} Methylation"), x = "PD PRS", color = "Sex") +
  theme_minimal()
ggplot(reg_all_data[na_row, ], aes(SCORE1_AVG, lumi::m2beta(betas_sub[age_hit$probe, ]), color = cut(age, quantile(age, seq(0, 1, 0.2)), include.lowest = TRUE))) +
  geom_point() +
  scale_color_viridis_d() +
  labs(color = "Age Quintile") +
  geom_smooth(method = "lm", se = FALSE) +
  labs(y = glue("{age_hit$probe} Methylation"), x = "PD PRS") +
  theme_minimal()
```
